...
# Infer the regulons:
adj_file=read.table("aracne_adj.txt")
aracne_reg=aracne2regulon("aracne_adj.txt", eset = as.matrix(counts_set), format = "3col") # The Mode of Regulation (MoR) between each gene pair of the adjacency file
# will be equal to the Spearman correlation coefficient between these genes. The likelihood of the interaction will be equal to: MI(gene pair)/max(MI).
...
ensembl_into_symbol=function(x) {mapIds(org.Hs.eg.db,x, "SYMBOL", "ENSEMBL", multiVals = "first")}
...
check.match= function(dataframe1, columns1, dataframe2, columns2) {
  ...
  # Detect matching pairs regardless their order:
  match_direction1=match(df1_interactions, df2_interactions)
  match_direction2=match(df1_interactions, df2_rev_interactions) 

  matches=c(match_direction1,match_direction2)
  matches=matches[!is.na(matches)]
  cat(length(matches),"matching rows","\n","The indices correspond to row indices of", substitute(dataframe2),"\n")
  return(matches)
}
```
